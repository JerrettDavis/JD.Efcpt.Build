...
internal sealed class SqlServerSchemaReader : ISchemaReader
{
    ...
    public SchemaModel ReadSchema(string connectionString)
    {
        ...
        // Get table list using sys.tables (more reliable for filtering)
    ...
    {
        // Use GetSchema with restrictions to get tables from dbo schema only
        // Restrictions array: [0]=Catalog, [1]=Schema, [2]=TableName, [3]=TableType
        var restrictions = new string?[4];
        restrictions[1] = "dbo"; // Only get tables from dbo schema
    ...
    private static IEnumerable<ColumnModel> ReadColumnsForTable(
        ...
        string tableName)
        => columnsData
            .Select($"TABLE_SCHEMA = '{schemaName}' AND TABLE_NAME = '{tableName}'", "ORDINAL_POSITION ASC")
            .Select(row => new ColumnModel(
                Name: row.GetString("COLUMN_NAME"),
                DataType: row.GetString("DATA_TYPE"),
                MaxLength: row.IsNull("CHARACTER_MAXIMUM_LENGTH") ? 0 : Convert.ToInt16(row["CHARACTER_MAXIMUM_LENGTH"]),
                Precision: row.IsNull("NUMERIC_PRECISION") ? 0 : Convert.ToByte(row["NUMERIC_PRECISION"]),
                Scale: row.IsNull("NUMERIC_SCALE") ? 0 : Convert.ToByte(row["NUMERIC_SCALE"]),
    ...
    private static DataTable GetIndexes(SqlConnection connection)
    {
        ...
        // Note: This provides basic index info; detailed properties like is_unique
        // and is_primary_key are not available through GetSchema
    ...
    private static DataTable GetIndexColumns(SqlConnection connection)
    {
        ...
        // Note: is_descending is not available, so all columns default to ascending order
    ...
    private static IEnumerable<IndexModel> ReadIndexesForTable(
        ...
        string tableName)
        => indexesData
            .Select($"table_schema = '{schemaName}' AND table_name = '{tableName}'")
            ...
            .Select(rowInfo => new
            {
                rowInfo.row,
                rowInfo.indexName,
                // GetSchema doesn't provide is_primary_key or is_unique, so default to false
    ...
    private static IEnumerable<IndexColumnModel> ReadIndexColumnsForIndex(
        ...
        => indexColumnsData.Select(
                $"table_schema = '{schemaName}' AND table_name = '{tableName}' AND index_name = '{indexName}'",
                "ordinal_position ASC")
            .Select(row => new IndexColumnModel(
                ColumnName: row.GetString("column_name"),
                OrdinalPosition: Convert.ToByte(row["ordinal_position"]),
