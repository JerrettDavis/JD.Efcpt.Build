...
{
    private sealed record ValidationContext(RecordingBuildLog Log);

    private static ValidationContext CreateContext() => new(new RecordingBuildLog());
    ...
    public async Task Warns_when_app_settings_receives_config_file()
    {
        await Given("a validator context", CreateContext)
            ...
            .Then("logs a warning about file type mismatch", ctx =>
                ctx.Log.WarnMessages.Any(w => w.Message.Contains("EfcptAppSettings received a .config file")))
            .And("suggests using EfcptAppConfig", ctx =>
                ctx.Log.WarnMessages.Any(w => w.Message.Contains("Consider using EfcptAppConfig")))
            .AssertPassed();
    }
    ...
    public async Task Warns_when_app_config_receives_json_file()
    {
        await Given("a validator context", CreateContext)
            ...
            .Then("logs a warning about file type mismatch", ctx =>
                ctx.Log.WarnMessages.Any(w => w.Message.Contains("EfcptAppConfig received a .json file")))
            .And("suggests using EfcptAppSettings", ctx =>
                ctx.Log.WarnMessages.Any(w => w.Message.Contains("Consider using EfcptAppSettings")))
            .AssertPassed();
    }
    ...
    public async Task No_warning_when_app_settings_receives_json_file()
    {
        await Given("a validator context", CreateContext)
            ...
            .Then("no warnings logged", ctx => ctx.Log.WarnMessages.Count == 0)
            .AssertPassed();
    }
    ...
    public async Task Handles_case_insensitive_extensions(string filePath, string parameterName)
    {
        await Given("a validator context", CreateContext)
            ...
            .Then("logs appropriate warning", ctx => ctx.Log.WarnMessages.Count == 1)
            .AssertPassed();
    }
}
